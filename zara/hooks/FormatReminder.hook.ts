...
ALGORITHM REQUIRED — DEPTH: FULL (CONCISE MODE)
Nothing escapes the Algorithm. Use 7-phase format BUT STRICTLY CONCISE:
- Start with: 🤖 PAI ALGORITHM header
- Each phase MAX 1 short sentence (OBSERVE, THINK, PLAN, BUILD, EXECUTE, VERIFY, LEARN)
- NO verbose thinking dumps, NO bulleted plans, NO multi-paragraph reasoning
- If you need user input, SKIP phases and ask the question directly
- TaskCreate only if task is genuinely multi-step (3+ real actions)
- End with voice line
User preference: uživatelka explicitly refuses long thinking/planning output. Respect it.
...
ALGORITHM REQUIRED — DEPTH: FULL (CONCISE MODE)
Nothing escapes the Algorithm. 7-phase format BUT STRICTLY CONCISE:
- 🤖 PAI ALGORITHM header
- Each phase MAX 1 short sentence
- NO verbose thinking dumps
- End with voice line
</system-reminder>`);
    process.exit(0);
  }
